...
app.post("/make-server-4cfee19e/chat/send", async (c: Context) => {
  try {
    ...
    const { hotelId, message, language, chatMessages, chatMessagesEn } = await c.req.json();
    ...
    // Store user message
    const chatKey = `chat:${session.userId}:${hotelId}`;
    const chatData = await kv.get(chatKey) || { messages: [], messageIndex: 0 };
    ...
    // Generate AI response - use hotel-specific messages if available
    const aiResponse = generateConciergeResponse(
      message, 
      language || 'ru', 
      chatMessages, 
      chatMessagesEn,
    ...
    // Save updated messageIndex to KV store
...
function generateConciergeResponse(
  ...
  language: string,
  chatMessages?: string[],
  chatMessagesEn?: string[],
  chatData?: { messageIndex: number }
): string {
  const lowerMessage = message.toLowerCase();

  // If hotel-specific messages are provided, use them sequentially
  if (chatMessages && chatMessages.length > 0) {
    const currentIndex = chatData?.messageIndex || 0;
    const messages = language === 'ru' && chatMessages ? chatMessages : 
                    (chatMessagesEn && chatMessagesEn.length > 0 ? chatMessagesEn : chatMessages);
    
    // Get message at current index and increment
    const response = messages[currentIndex % messages.length];
    
    // Update the index in chatData if provided
    if (chatData) {
      chatData.messageIndex = (currentIndex + 1) % messages.length;
    }
