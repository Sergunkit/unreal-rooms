  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className={`absolute inset-0 blur-2xl -z-10 transition-all duration-500 ${
          ...
          >
            <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-2xl" xmlns="http://www.w3.org/2000/svg">
              <defs>
                ...
                <filter id="shadow"><feDropShadow dx="0" dy="4" stdDeviation="3" floodOpacity="0.3" /></filter>
              </defs>
              <path d="M 35 25 Q 35 18 40 18 L 60 18 Q 65 18 65 25" fill="none" stroke="#654321" strokeWidth="3" strokeLinecap="round" />
              <rect x="20" y="30" width="60" height="50" rx="4" fill="url(#leatherGradient)" filter="url(#shadow)" stroke="#654321" strokeWidth="2" />
              ...
              <rect x="47" y="48" width="6" height="8" rx="1" fill="#C9A961" stroke="#8B7355" strokeWidth="1" />
                ...
                {collectedArtefacts.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-muted-foreground mt-2">
                      {language === 'ru' ? 'Находите артефакты в отелях!' : 'Find artefacts in hotels!'}
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-4">
                    {collectedArtefacts.map((artefact) => (
                      <div key={artefact.artefactId} className="bg-secondary/50 rounded-lg p-3 text-center">
                        {artefact.image && (
                          <img src={artefact.image} alt={language === 'ru' ? artefact.name : artefact.nameEn} className="w-full h-16 object-cover rounded-md mb-2" />
                        )}
